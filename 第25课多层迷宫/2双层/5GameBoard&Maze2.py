...
class Board:
    def __init__(self):
        ...
        self.walls_2 = pygame.sprite.Group()  # 将墙作为类
        # ---------------一层-------------------
        self.lines = 1  # 层数
        self.A = Data.A
        self.wall_s(self.walls, self.A)  # 第一层
        #-------------二层--------------------
        self.B=Data.B
        self.wall_s(self.walls_2,self.B)
    ...
    def wall_s(self, walls, A):  # 添加墙精灵组
        for i in range(len(A)):
            k = Maze.Wall(self.screen, A[i][0], A[i][1], A[i][2], A[i][3])
            walls.add(k)  # 添加到组
