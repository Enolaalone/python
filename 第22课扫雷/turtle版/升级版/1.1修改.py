...
class Mineboard:
                ...
                for j in range(self.col):
                    if self.mines[i][j].x - self.mine_size / 2 < x < self.mines[i][j].x + self.mine_size / 2 and \
                        ...
                        if self.mines[i][j].click_r:  # 右键检测
                            ...
                            RecordMine(self.click_times,-self.width / 2, 6 * self.mine_size + 1)
                            # self.mines_times()
    ...
    def Tstart(self):
        ...
        # 创建一个新线程
        self.t1 = threading.Thread(target= RecordTime(self.width / 2 - 4 * self.mine_size, 6 * self.mine_size + 1, self.stop_event).time_1())
        self.t1.start()
