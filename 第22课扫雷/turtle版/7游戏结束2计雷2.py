...
b1=t.Pen()
c1=t.Pen()
def writer(x,y,content,size):#写数字
    t.seth(-90)
    t.pu()
    t.goto(x,y)
    t.pd()
    t.color('red')
    t.write(content,align='center',font=('宋体',size,'bold'))
    t.color('black')
    t.seth(0)
def emoji(a,shape):
    global height
...
b1.hideturtle()
#--------计雷器------------
cell2(-width/2,height/2)
writer(-width/2+mine_size,height/2-2*mine_size,mine_number,60)
#-------计时器------------
