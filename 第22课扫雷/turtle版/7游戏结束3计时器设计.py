...
e1=t.Pen()
t.hideturtle()
def writer(a,x,y,content,size):#写数字
    t.tracer(0)
    a.hideturtle()
    a.seth(-90)#写字方向
    a.pu()
...
b1.hideturtle()
#--------计雷器------------
cell2(-width/2,height/2)#黑框
writer(e1,-width/2+mine_size,height/2-2*mine_size,mine_number,50)
#-------计时器------------
cell2(width/2-4*mine_size,height/2)
...
def time_k():
    ...
    while 1:
        time2=tm.time()
        now=time2-time1
        writer(d1,width/2-4*mine_size+2*mine_size,height/2-2*mine_size,f'{now:.0f}s',50)
        t.tracer(0)
        tm.sleep(0.2)
        d1.clear()
        t.tracer(1)
