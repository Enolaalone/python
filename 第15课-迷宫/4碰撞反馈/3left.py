...
D=[-400,-284,-32,32]
D1=[[-280,-212],[-180,-116],[-80,-16],[20,84],[120,184],[220,400]]
D2=[[-212,180,-88], [-16,20,-92],[184,216,-92]]
D3=[[-116,-80,92],[84,120,92]]
def move_down():
    a.up()
...
def move_left():
    if (D[0][0]<=a.xcor()<=D[0][1] and D[0][3]<=a.ycor()<=D[0][3]) or (D[i][0]<=a.xcor()<=D[0][1]):
        a.up()
        a.goto(a.xcor()-4,a.ycor())
    for i in range(len(D1)):
        if (D[i][0]<=a.xcor()<=D[i][1]):
            a.up()
            a.goto(a.xcor() - 4, a.ycor())
